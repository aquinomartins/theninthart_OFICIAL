...
use Tna\Story\StoryResolverInterface;
...
final class StoryRunService
{
    ...
    public function __construct(private readonly TransactionManager $tx, private readonly StoryResolverInterface $resolver = new WeightedNarrativeResolver()){}
    ...
    { return $this->tx->run(function(PDO $pdo) use($body,$token,$key): array {
        ...
        $controls=$this->controls($pdo,(int)$session['id']); $widgets=$this->widgets($pdo,(int)$session['id']);
        $resolved=$this->resolver->resolve($pdo,(int)$session['seed'],$controls,$widgets,$revision); $now=$this->now(); $publicId=$this->id('run');
        $input=['sessionId'=>$sessionId,'revision'=>$revision,'controls'=>$controls,'widgets'=>$widgets];
        ...
        $stmt->execute([$publicId,$session['id'],$session['user_id'],$resolved['dominantVersionId'],'1.0.0','1.0.0',$resolved['resolvedState']['rulesVersion']??'baseline-v1',$resolved['resolutionMode'],Json::encode($input),Json::encode($resolved['resolvedState']),Json::encode(['blocks'=>$resolved['blocks']]),Json::encode($resolved['trace']),(int)$session['seed'],$revision,$key,$now,$now]);
    ...
    private function formatRun(PDO $pdo,array $run): array { $p=$pdo->prepare('SELECT panel_payload_json FROM story_run_panels WHERE story_run_id=? ORDER BY position'); $p->execute([$run['id']]); $sels=[]; foreach($p->fetchAll() as $row){$sels[]=json_decode($row['panel_payload_json'],true)?:[];} return ['storyRunId'=>$run['public_id'],'title'=>(json_decode($run['resolved_state_json'],true)['title']??BaselineStoryResolver::TITLE),'resolutionMode'=>$run['resolution_mode'],'revision'=>(int)$run['revision'],'seed'=>(int)$run['seed'],'versions'=>['dominant'=>(json_decode($run['resolved_state_json'],true)['dominantVersion']??BaselineStoryResolver::DOMINANT_VERSION)],'resolvedState'=>json_decode($run['resolved_state_json'],true)?:[],'trace'=>json_decode($run['resolution_trace_json'],true)?:[],'selections'=>$sels,'createdAt'=>$run['created_at'],'updatedAt'=>$run['updated_at']]; }
