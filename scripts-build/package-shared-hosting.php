...
function check_all(bool $releaseMode=false): void {
    ...
    foreach (array_merge($assets['scripts'] ?? [], $assets['styles'] ?? []) as $asset) { if (!is_file($root . '/assets/story-engine/' . $asset)) { err('Bundle Angular referenciado não existe: ' . $asset); } if (str_contains($asset, '.map')) { err('Source map público no manifesto Angular: ' . $asset); } }
